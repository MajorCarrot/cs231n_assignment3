...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            h_i, cache_i = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        else:
            h_i, cache_i = rnn_step_forward(x[:, i, :], h[:, i-1, :], Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T):
        dx_i, dh_i, dWx_i, dWh_i, db_i = rnn_step_backward(dh[:, T-i-1, :] + dh_i, cache[T-i-1])
        if i == T-1: dh0 = dh_i
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, x, prev_c, prev_h, Wx, Wh, b, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i, f, o, g, x, prev_c, prev_h, Wx, Wh, b, next_c, next_h = cache
    _, H = next_h.shape
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            h_i, c_i, cache_i = lstm_step_forward(x[:, i, :], h0, c_i, Wx, Wh, b)
        else:
            h_i, c_i, cache_i = lstm_step_forward(x[:, i, :], h[:, i-1, :], c_i, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T):
        dx_i, dh_i, dc_i, dWx_i, dWh_i, db_i = lstm_step_backward(dh[:, T-i-1, :] + dh_i, dc_i, cache[T-i-1])
        if i == T-1: dh0 = dh_i
