...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k_den, l_k_N_den = 0, 0
        for j in range(2*N):
            if j != k:
                l_k_den += torch.exp(sim(z_k, out[j]) / tau)
            if j != (k+N):
                l_k_N_den += torch.exp(sim(z_k_N, out[j]) / tau)

        l_k = torch.exp(sim(z_k, z_k_N) / tau) / l_k_den
        l_k_N = torch.exp(sim(z_k_N, z_k) / tau) / l_k_N_den
        total_loss -= (torch.log(l_k) + torch.log(l_k_N))
...
def sim_positive_pairs(out_left, out_right):
    ...
    out_left_normalized = out_left / torch.linalg.norm(out_left, dim=1, keepdim=True)
    out_right_normalized = out_right / torch.linalg.norm(out_right, dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = out_norm.matmul(out_norm.T)
