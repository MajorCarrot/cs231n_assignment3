...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Apply the reshaping and switch dimensions 2 and 3 (for each head)
        query = self.query(query).view(N, -1, heads, head_dim)
        key = self.key(key).view(N, -1, heads, head_dim)
        value = self.value(value).view(N, -1, heads, head_dim)
